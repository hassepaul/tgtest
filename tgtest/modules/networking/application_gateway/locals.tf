locals {
  ...
  custom_probes = {
    for key, value in var.application_gateway_applications : key => merge({ name = value.name }, value.custom_probe)
    if lookup(value, "custom_probe", false) != false
  }
    ...
    flatten(
      [
        for app_key, config in var.application_gateway_applications : [
          ...
          if lookup(value, "https_redirect", false) != false
        ]
      ]
    ) : format("%s-%s", listener.app_key, listener.listener_key) => listener.value    
  }
