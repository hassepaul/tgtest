...
locals {
  ...
  az = {
    ...
    app_service = {
      name        = substr(join("-", compact([local.prefix, "web", local.suffix])), 0, 60)
      name_unique = substr(join("-", compact([local.prefix, "web", local.suffix_unique])), 0, 60)
      dashes      = true
      slug        = "web"
      min_length  = 2
      max_length  = 60
    ...
    data_factory_linked_service_blob_storage = {
      name        = substr(join("-", compact([local.prefix, "adfsvst", local.suffix])), 0, 260)
      name_unique = substr(join("-", compact([local.prefix, "adfsvst", local.suffix_unique])), 0, 260)
      dashes      = true
      slug        = "adfsvst"
  ...
  validation = {
    ...
    data_factory_linked_service_blob_storage = {
      valid_name        = length(regexall(local.az.data_factory_linked_service_blob_storage.regex, local.az.data_factory_linked_service_blob_storage.name)) > 0 && length(local.az.data_factory_linked_service_blob_storage.name) > local.az.data_factory_linked_service_blob_storage.min_length
      valid_name_unique = length(regexall(local.az.data_factory_linked_service_blob_storage.regex, local.az.data_factory_linked_service_blob_storage.name_unique)) > 0
    }
    data_factory_pipeline = {
